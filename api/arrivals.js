// api/arrivals.js — Tiempos de llegada con caché compartida en servidor
// Node.js runtime (no edge) para poder mantener estado en memoria entre peticiones

// ─── Caché compartida (module-level: sobrevive warm starts en Vercel) ──────────
const arrivalsCache = new Map(); // codStop → { data, timestamp }
const CACHE_TTL = {
  '6': 20 * 1000,  // EMT urbanos: 20 segundos
  '8': 30 * 1000,  // CRTM interurbanos: 30 segundos
};
  ...
  for (const url of urls) {
    try {
      ...
      if (data?.accessToken) {
        ...
        console.log('[arrivals] EMT token renovado');
        return emtToken;
      }
...
async function fetchEmtArrivals(stopId) {
  ...
  // Token caducado → renovar y reintentar una vez
...
// ─── Obtener tiempos de CRTM (modo 8) ─────────────────────────────────────────
async function fetchCrtmArrivals(codStop) {
  const url = `https://www.crtm.es/widgets/api/GetStopsTimes.php?codStop=${codStop}&type=0&orderBy=2&stopTimesByIti=${codStop}&_=${Date.now()}`;
...
export default async function handler(req, res) {
  ...
  const mode = codStop.split('_')[0]; // '6' o '8'
  ...
  if (cached && Date.now() - cached.timestamp < ttl) {
    ...
    console.log(`[arrivals] Cache hit para ${codStop} (${age}s)`);
  ...
  try {
    ...
    if (mode === '6') {
      ...
      arrivals = await fetchEmtArrivals(stopId);
    } else {
      arrivals = await fetchCrtmArrivals(codStop);
    }

    arrivalsCache.set(codStop, { data: arrivals, timestamp: Date.now() });
    console.log(`[arrivals] Fetch fresco para ${codStop}: ${arrivals.length} llegadas`);

    return res.status(200).json({ arrivals, cached: false, age: 0, error: false });

  } catch (error) {
    console.error(`[arrivals] Error para ${codStop}:`, error.message);

    // Fallback: servir caché stale si existe
    if (cached) {
      const age = Math.round((Date.now() - cached.timestamp) / 1000);
      console.warn(`[arrivals] Sirviendo caché stale (${age}s) para ${codStop}`);
