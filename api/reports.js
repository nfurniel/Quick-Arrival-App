...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const params = `select=id,type,metadata,description,line_name,created_at,report_votes(vote_type,user_id)&status=eq.active&line_name=eq.${encodeURIComponent(lineName)}&created_at=gte.${since}&order=created_at.desc&limit=20`;
  ...
  if (req.method === 'POST') {
    ...
    const { type, metadata, description, lat, lng, lineName } = req.body || {};
    ...
    // Duplicado: mismo usuario, mismo tipo, misma línea en las últimas 2h
    const since = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString();
    const dupCheck = await fetch(
      `${SUPABASE_URL}/rest/v1/reports?user_id=eq.${user.id}&type=eq.${type}&line_name=eq.${encodeURIComponent(cleanLineName)}&status=eq.active&created_at=gte.${since}&select=id&limit=1`,
      { headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` } }
    );
    if (dupCheck.ok) {
